...
void Graph::shortest_path_dag() {
    ...
    // to source as 0
    fill(distance2src.begin(), distance2src.end(), FINF);
    //memset(&distance2src[0], 1000000, distance2src.size()* sizeof(distance2src[0]));

//    for (int i = 0; i < this->num_nodes_; i++)
//        distance2src[i] = FINF;
//    cout << distance2src.size()<<endl;
...
//             in != V_[i].successor_idx.end(); in++, ie++) {
        for (j = 0; j < V_[i].successor_idx.size(); j++) {
            in = V_[i].successor_idx[j];
            ie = V_[i].successor_edges_idx[j];
...
void Graph::flip_path() { // erase the best one link to sink
    ...
    int ie;
    vector<int>::iterator edge_id_it_suc;
...
//    memset(&visited[0], false, num_nodes_ * sizeof(bool));
    ...
    set<int> s( update_node_id.begin(), update_node_id.end() );
    while (!s.empty()) {
        int i = *s.begin();
        s.erase(s.begin());
        for (auto &&j : V_[i].successor_idx) {
            if (parent_node_id[j] == i) {
                s.insert(j);
                update_node_id.push_back(j);
                distance2src[j] = FINF;
            }
        }
        //cout << tmp_q.size()<<endl;
    }
    // remove duplicate
    set<int> output_set( update_node_id.begin(), update_node_id.end() );
    update_node_id.assign( output_set.begin(), output_set.end() );
    sort(update_node_id.begin(), update_node_id.end());
    if (update_node_id[update_node_id.size()-1] == sink_id_)
        update_node_id.pop_back();
}
...
void Graph::update_shortest_path_tree_recursive(vector<int> &update_node_id) {
    upt_node_num = update_node_id.size();
    for (int i = 0; i < update_node_id.size(); i++) {
        int cur_node = update_node_id[i];
        if (cur_node == sink_id_){
            continue;
        }
        for (int j = 0; j < V_[cur_node].precursor_idx.size(); j++) {
            int it = V_[cur_node].precursor_idx[j];
            if(it > cur_node)
                continue;

            int ie = V_[cur_node].precursor_edges_idx[j];
            double re_cal_edge_w = distance2src[it] + edge_weights[ie];
    ...
    vector<int> tmp_flat_paths;
    for (int i=0;i<path_set.size();i++){
        for (int j=0;j<path_set[i].size();j++){
            tmp_flat_paths.push_back(path_set[i][j]);
        }
    }
    set<int> flat_paths(tmp_flat_paths.begin(), tmp_flat_paths.end());

    node_upt_waitinglist.insert(make_pair(distance2src[sink_id_], sink_id_));
    for(auto it=flat_paths.begin(); it!=flat_paths.end(); ++it){
        if(*it == src_id_ || *it == sink_id_){
            continue;
        }
        node_upt_waitinglist.insert(make_pair(distance2src[*it], *it));
    }
...
//        cout <<distance2src[156004]<<endl;
        ...
        for (int j = 0; j < V_[curr_node_id].successor_idx.size(); j++) {
            int it = V_[curr_node_id].successor_idx[j];
            int ie = V_[curr_node_id].successor_edges_idx[j];
            double cur_edge_weight = edge_weights[ie] + curr_node_dist;
            if (cur_edge_weight  - distance2src[it] < -0.0000001) {
                ...
                distance2src[it] = cur_edge_weight;
                // no need to update ancestor in this condition
                //Set the new distance and add to map
                node_upt_waitinglist.insert(make_pair(cur_edge_weight, it));
                num_heap_operation ++;
                //if (node_upt_waitinglist.size() > upt_node_num)
                //    max_heap_size = node_upt_waitinglist.size();
            }
        }
    }
