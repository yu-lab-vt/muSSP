...
void Sink::sink_update_all(vector<double> &distance2src, int sink_id, int n){
    double sink_dist = sink_cost_ - distance2src[sink_id];
    for(int i=2; i<n; i+=2){
        sink_precursor_weights[i] = sink_dist + distance2src[i];
        if(sink_precursor_weights[i] + distance2src[sink_id] <0)
            sink_precursors.insert(make_pair(sink_precursor_weights[i], i));
    }
}

void Sink::sink_update_all_weight(vector<double> &distance2src, int sink_id, int n){
    double sink_dist = sink_cost_ - distance2src[sink_id];
    for(int i=2; i<n; i+=2){
        sink_precursor_weights[i] = sink_dist + distance2src[i];
    }
...
void Sink::sink_build_precursormap(vector<double> &ancestor_ssd, vector<int> &ancestor_node_id, vector<int> &parent_node_id, int n){
...
//            sink_precursors.insert(make_pair(sink_precursor_weights[i], i));
//            //sink_precursors.insert(make_pair(sink_precursor_weights[parent_node_id[parent_node_id[i]]], parent_node_id[parent_node_id[i]]));
//        }
//    }
    ...
    sink_precursors.insert(sink_precursors.end(), make_pair(FINF, 0)); // worst case
}
void Sink::sink_update_all_half(vector<double> distance2src, int sink_id, int n){
    double sink_dist = distance2src[sink_id];
    double cur_dist;
    for(int i=1; i<n-1; i++){
        cur_dist = sink_cost_ + distance2src[i] - sink_dist;
        sink_precursors.insert(make_pair(cur_dist, i));
        sink_precursor_weights[i] = cur_dist;
    }

}
// this is re-write in Graph class, so not used any more here
void Sink::sink_update_subgraph(vector<int> update_node_id, vector<double> distance2src, int n, int sink_id){
