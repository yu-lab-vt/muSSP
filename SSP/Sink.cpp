...
void Sink::sink_update_all(std::vector<double> &distance2src, int sink_id, int n){
...
void Sink::sink_update_all_weight(std::vector<double> &distance2src, int sink_id, int n){
    double sink_dist = sink_cost_ - distance2src[sink_id];
    for(int i=2; i<n; i+=2){
        sink_precursor_weights[i] = sink_dist + distance2src[i];
    }
