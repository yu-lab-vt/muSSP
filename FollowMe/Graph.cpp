...
void Graph::find_node_set4update(std::vector<int> &update_node_id)
{
    ...
    std::set<int> s( update_node_id.begin(), update_node_id.end() );
    while (!s.empty()) {
        int i = *s.begin();
        s.erase(s.begin());
        ...
        //cout << tmp_q.size()<< std::endl;
    }
    // remove duplicate
    std::set<int> output_set( update_node_id.begin(), update_node_id.end() );
...
void Graph::update_shortest_path_tree_recursive(std::vector<int> &update_node_id) {
    ...
    for (size_t i = 0; i < update_node_id.size(); i++) {
    ...
    std::vector<int> tmp_flat_paths;
    for (size_t i=0;i<path_set.size();i++){
        for (size_t j=0;j<path_set[i].size();j++){
            tmp_flat_paths.push_back(path_set[i][j]);
        }
    }
    std::set<int> flat_paths(tmp_flat_paths.begin(), tmp_flat_paths.end());

    node_upt_waitinglist.insert(std::make_pair(distance2src[sink_id_], sink_id_));
    for(auto it=flat_paths.begin(); it!=flat_paths.end(); ++it){
        if(*it == src_id_ || *it == sink_id_){
            continue;
        }
        node_upt_waitinglist.insert(std::make_pair(distance2src[*it], *it));
    }
...
//        cout <<distance2src[156004]<< std::endl;
        ...
        for (size_t j = 0; j < V_[curr_node_id].successor_idx.size(); j++) {
            ...
            if (cur_edge_weight  - distance2src[it] < -0.0000001) {
                ...
                node_upt_waitinglist.insert(std::make_pair(cur_edge_weight, it));
                num_heap_operation ++;
